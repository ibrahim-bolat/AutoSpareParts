            ...
            await using (var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>())
            {
                if (unitOfWork != null)
                {
                    try
                    {
                        ...
                        {
                            var endPoints = controller.GetMethods().Where(m => m.IsDefined(typeof(AuthorizeEndpointAttribute)));

                            foreach (var endPoint in endPoints)
                            {
                                var attributes = endPoint.GetCustomAttributes(true);

                                var authorizeDefinitionAttribute = attributes.FirstOrDefault(a => a.GetType() == typeof(AuthorizeEndpointAttribute)) as AuthorizeEndpointAttribute;
