...
namespace AutoSpareParts.Application.Extensions;

public static class EndpointsMigrateExtension
{
            ...
            await using (var unitOfWork = scope.ServiceProvider.GetRequiredService<IUnitOfWork>())
            {
                if (unitOfWork != null)
                {
                    try
                    {
                        ...
                        {
                            var endPoints = controller.GetMethods().Where(m => m.IsDefined(typeof(EndpointAttribute)));

                            foreach (var endPoint in endPoints)
                            {
                                var attributes = endPoint.GetCustomAttributes(true);

                                var authorizeDefinitionAttribute = attributes.FirstOrDefault(a => a.GetType() == typeof(EndpointAttribute)) as EndpointAttribute;


                                if (authorizeDefinitionAttribute != null)
                                {
                                    var controllerAttributes = controller.GetCustomAttributes(true);
                                    var areaAttribute = controllerAttributes.FirstOrDefault(a => a.GetType() == typeof(AreaAttribute)) as AreaAttribute;
                                    ...
                                    if (!endpoints.Any(e => e.Code == code))
                                    {
                                        ...
                                        newEndpoint = new()
                                        {
                                            ...
                                            AreaName = areaName,
                                            EndpointType = Enum.GetName(typeof(EndpointType), authorizeDefinitionAttribute.EndpointType),
                                            HttpType = httpType,
