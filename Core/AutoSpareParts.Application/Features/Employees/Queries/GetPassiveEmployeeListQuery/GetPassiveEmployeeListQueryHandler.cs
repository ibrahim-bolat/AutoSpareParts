...
public class GetPassiveEmployeeListQueryHandler:IRequestHandler<GetPassiveEmployeeListQueryRequest,GetPassiveEmployeeListQueryResponse>
{
    private readonly UserManager<Employee> _employeeManager;
    private readonly IMapper _mapper;

    public GetPassiveEmployeeListQueryHandler(UserManager<Employee> employeeManager, IMapper mapper)
    {
        _employeeManager = employeeManager;
        _mapper = mapper;
    }

    public Task<GetPassiveEmployeeListQueryResponse> Handle(GetPassiveEmployeeListQueryRequest request, CancellationToken cancellationToken)
    {
        var employeeData = _employeeManager.Users.Where(u=>u.IsDeleted==true).AsQueryable();
        ...
        List<EmployeeSummaryDto> passiveEmployee = _mapper.Map<List<EmployeeSummaryDto>>(data);
        var response = new DatatableResponseDto<EmployeeSummaryDto>
        {
            Draw = request.DatatableRequestDto.Draw,
            RecordsTotal = recordsTotal,
            RecordsFiltered = recordsTotal,
            Data = passiveEmployee
        };
        return Task.FromResult(new GetPassiveEmployeeListQueryResponse{
