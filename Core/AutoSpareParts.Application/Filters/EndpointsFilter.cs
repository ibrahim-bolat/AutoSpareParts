...
public class EndpointsFilter : IAsyncActionFilter
{
    private readonly UserManager<AppUser> _userManager;
    private readonly IUnitOfWork _unitOfWork;

    public EndpointsFilter(UserManager<AppUser> userManager, IUnitOfWork unitOfWork)
    {
    ...
    private bool IpCheck(string remoteIpAddress, Endpoint endpoint)
    {
        ...
        {
            List<Domain.Entities.IPAddress> whiteList = endpoint.IPAddresses.Where(i => i.IPListType == IPListType.WhiteList && i.IsActive).ToList();
            List<Domain.Entities.IPAddress> blackList = endpoint.IPAddresses.Where(i => i.IPListType == IPListType.BlackList && i.IsActive).ToList();
            if (whiteList.Count != 0 && blackList.Count != 0)
            {
                if (whiteList.Any(i => IPAddressRange.Parse($"{i.RangeStart} - {i.RangeEnd}").Contains(System.Net.IPAddress.Parse(remoteIpAddress))) && 
                    !blackList.Any(i => IPAddressRange.Parse($"{i.RangeStart} - {i.RangeEnd}").Contains(System.Net.IPAddress.Parse(remoteIpAddress))))
                {
                    return true;
                }
                return false;
            ...
            if (whiteList.Count == 0 && blackList.Count != 0)
            {
                if (!blackList.Any(i => IPAddressRange.Parse($"{i.RangeStart} - {i.RangeEnd}").Contains(System.Net.IPAddress.Parse(remoteIpAddress))))
                {
                    return true;
                }
                return false;
