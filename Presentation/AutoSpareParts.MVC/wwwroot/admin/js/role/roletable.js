...
$(document).ready(function ($) {
                ...
                "data": "Id","className": "text-center","width": "50px", "render": function (data, type, row, meta) {
                    if(row.Status){
                        if(data === 1 || data === 2 || data === 3){
                            return'<a class="btn btn-primary mr-2" href="endpoint/index"><i class="fa fa-tasks">Yetkilendirme</i></a>' +
                                '<a class="btn btn-info mr-2" href="role/employeesofrole/' + data + '"><i class="fa fa-info-circle">Rolün Çalışanları</i></a>' +
                            ...
                            return'<a class="btn btn-primary mr-2" href="endpoint/index"><i class="fa fa-tasks">Yetkilendirme</i></a>' +
                                ...
                                '<a class="btn btn-danger" onclick="setPassiveRole(' + data + ')"><i class="fa fa-solid fa-times">Pasif Yap</i></a>';
                        }
                    }
                    else{
                        return '<a class="btn btn-primary mr-2" href="endpoint/index"><i class="fa fa-tasks">Yetkilendirme</i></a>' +
                            '<a class="btn btn-info mr-2" href="role/employeesofrole/' + data + '"><i class="fa fa-info-circle">Rolün Çalışanları</i></a>' +
                        '<a class="btn btn-secondary mr-2" onclick="getByIdforUpdate(' + data + ')"><i class="fa fa-pencil-square-o">Rol Güncelle</i></a>' +
                        '<a class="btn btn-success" onclick="setActiveRole(' + data + ')"><i class="fa fa-solid fa-check">Aktif Yap</i></a>';
                    }
                }
            }
        ],
    ...
    //Modal Form Create
    $('#createRoleModalForm').on('submit', '#createModalForm', function () {
    ...
    //Modal Form Update
    $('#updateRoleModalForm').on('submit', '#updateModalForm', function () {
        var data = $(this).serialize();
        $.ajax({
...
function setActiveRole(Id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/role/setactiverole/' + Id,
                type: "POST",
                contentType: "application/json;charset=UTF-8",
                dataType: "json",
...
function setPassiveRole(Id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: '/admin/role/setpassiverole/' + Id,
                type: "POST",
                contentType: "application/json;charset=UTF-8",
                dataType: "json",
                success: function (activeResult) {
