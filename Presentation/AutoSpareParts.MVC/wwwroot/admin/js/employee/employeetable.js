...
$(document).ready(function ($) {
    var tables = $("#employeeTable").DataTable({
        "pageLength": 10,
        "ordering": true,
        "order": [[0, "asc"]],
        "info": true,
        ...
        "columns": [
            ...
            {"data": "UserName", "name": "Çalışan Adı", "autoWidth": true},
            {"data": "Email", "name": "Email", "autoWidth": true},
            {
                "data": "Id", "width": "50px", "render": function (data) {
                    return '<a class="btn btn-success mr-2" onclick="return getRole(' + data + ')"><i class="fa fa-tasks">Rol Ata</i></a>' +
                        '<a class="btn btn-info mr-2" href="employeeaccount/profile/' + data + '"><i class="fa fa-info-circle">Detay</i></a>' +
                        '<a class="btn btn-secondary mr-2" href="employeeaccount/editprofile/'+data+'"><i class="fa fa-pencil-square-o">Profil Güncelle</i></a>' +
                        '<a class="btn btn-warning mr-2 editPasswordButton" data-employeeid="'+data+'"><i class="fa fa-pencil-square-o">Şifre Güncelle</i></a>' +
    ...
    //Modal Form Create
    $('#createEmployeeModalForm').on('submit', '#createModalForm', function () {
    ...
    //Modal Form Delete
    $('#deleteEmployeeModalForm').on('submit', '#deleteModalForm', function (e) {
        e.preventDefault();
        var Id = $('#deleteID').val();
        $.ajax({
    ...
    $('#employeeTable').on('click', '.editPasswordButton', function (e) {
        e.preventDefault();
        var Id = $(this).data("employeeid");
        $.ajax({
            url: '/admin/employee/geteditemployeepasswordbyid',
            type: 'GET',
            data: { "Id": Id},
            dataType: 'html',
            success: function (modal) {
                $("#editEmployeePasswordModalPartial").html(modal);
                $("#editEmployeePasswordModal").modal("show");
            },
            error: function (errormessage) {
                toastMessage(3000, "error", "Hata","Çalışan Bilgisi Getirilemedi");
            }
        });
        return false;
    });
    ...
    $('#editEmployeePasswordModalPartial').on('submit', '#editEmployeePasswordModalForm', function () {
        var data = $(this).serialize();
        $.ajax({
            url: "/admin/employee/editemployeepassword",
            type: "POST",
            data: data,
            success: function (result) {
                if (result.success) {
                    $('#editEmployeePasswordModal').modal('hide');
                    toastMessage(3000, "success", "Tebrikler","Şifre Başarıyla Güncellendi.");
                } else {
                    var mytag=$('<div></div>').html(result);
                    $('#editEmployeePasswordModalFormModalBody').html(mytag.find(".modal-body").html());
                }
            },
            error: function (errormessage) {
    ...
    $('#roleModalSave').click( function (e) {
        ...
        $.ajax({
            url: '/admin/employee/assignrolelisttoemployee/' + Id,
            type: "POST",
            data: postData,
            dataType: "json",
            traditional: true,
    ...
    if(app.ToastMessages.registerMessage==="True"){
        toastMessage(3000,"success","Çalışan Kaydedildi.",
            "Çalışan Kaydetme İşlemi Başarıyla Gerçekleştirildi.");
    }
    if(app.ToastMessages.editProfileMessage==="True"){
        toastMessage(3000,"success","Çalışan Güncellendi.",
            "Çalışan Güncelleme İşlemi Başarıyla Gerçekleştirildi.");
    }
});
...
function getRole(Id) {
    var html ="";
    $.ajax({
        url: '/admin/employee/getrolelistbyemployeeid/' + Id,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
...
function getByIdforDelete(Id) {
    ...
    $.ajax({
        url: '/admin/employee/getemployeesummarybyid/' + Id,
        type: "GET",
        contentType: "application/json;charset=UTF-8",
        dataType: "json",
        success: function (result) {
            if (result.success) {
                $('#deleteID').val(result.employee.Id);
                $('#deleteFirstName').val(result.employee.FirstName);
                $('#deleteLastName').val(result.employee.LastName);
                $('#deleteUserName').val(result.employee.UserName);
                $('#deleteEmail').val(result.employee.Email);
